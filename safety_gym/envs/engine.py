...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def obs_lidar_natural(self, group):
        ...
        grp = np.asarray([i == group for i in range(int(const.NGR - OUP))], dtype='uint8')
...
class RandomizeEngine(gym.Env, gym.utils.EzPickle):
    ...
    def __init__(self, **kwargs):
        ...
        self.randomize_config_path = kwargs['randomize_config_path']
        self.parse(config)
        gym.utils.EzPickle.__init__(self, config=config)
    ...
    def build(self):
        ...
        if self.world is None:
            self.world = World(self.world_config_dict, render_context=None,
                               randomize_config_path=self.randomize_config_path)
            self.world.reset()
            self.world.build()
        else:
            self.world.reset(build=False)
